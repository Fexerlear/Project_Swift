...
extension Date {
    ...
    // 获取下个月最后一天
    func getNextDateDay() ->Date {
        let calendar = NSCalendar.current
        var com = calendar.dateComponents([.year,.month,.day], from: self)
        com.month = com.month! + 1
        com.day = 4
        let nextMonth = calendar.date(from: com)
        let nextMonthDay = nextMonth?.countOfDaysInMonth()
        com.day = nextMonthDay
        return calendar.date(from: com)!
...
class YLDateClass {
    ...
    //MARK: 是否是闰年
    /// 是否是闰年
    static func is_ALeapYear(year: Int) -> Bool {
...
//        dateFormatter.timeZone = TimeZone(abbreviation: "UTC")
    ...
    /// - Returns: 返回所要的日期
    static func getCurrentDateToWantDate(year:Int,month:Int,day:Int)->Date {
        let current = Date()
